...
let () =
  ...
  Printf.printf "Good radars: 5:%s 20:%s\n"
    (k_nn radars 5 (good_1, ""))
    (k_nn radars 20 (good_2, ""));
  Printf.printf "Bad radars: 10:%s 40:%s\n"
    (k_nn radars 10 (bad_1, ""))
    (k_nn radars 40 (bad_2, ""));
  print_string "Random radars:";
  for i = 0 to 4 do
    let k = Random.int 100 in
    Printf.printf " %d:%s" k (k_nn radars k (List.nth random_radars i, ""))
  done;
  print_newline ()
